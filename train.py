...
def train(train_loader, model, optimizer, scheduler, epoch, criteria_loss, writer, device, total_step):
    ...
    latest_save_file = os.path.join(save_path, 'iESegNet_latest.pth')
    torch.save(checkpoint, latest_save_file)

    # 正常按频率保存纯模型权重（用于后续测试和推理）
    if (epoch + 1) % cfg.save_freq == 0:
        save_file = os.path.join(save_path, 'iESegNet-%d.pth' % epoch)
...
if __name__ == '__main__':
    ...
    # ================= 功能2：断点续接功能 =================
    start_epoch = 0
    latest_save_file = os.path.join(cfg.save_path, 'iESegNet_latest.pth')
